...
def train(data_loader, type, epoch, model):
    ...
    # Perform sampling
    if type == "Test":
        generated_graphs = model.sample_graphs(num=SAMPLES)
        # print(f"Generated {generated_graphs} graph.")
        # mlflow.log_metric(key=f"Sampled graph", value=float(generated_graphs), step=epoch)
