...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def apply_noise(self, Feat):
        ...
        #-----------------------For saving more samples --------------------------------------
        # F_out = []
        # for i in range(0,2):
        #     sample_ds= diffusion_utils.sample_discrete_features(probF=probF)
        #     F_ds = torch.FloatTensor(Feat.shape).to("cuda:0")
        #     for i in range(0, F_ds.shape[0]):
        #         for j in range(0, F_ds.shape[1]):
        #             F_ds[i][j] = Feat[i][j][sample_ds.Feat[i][j]]
        #             F_out.append(F_ds[i][j].cpu().detach().numpy().tolist())
