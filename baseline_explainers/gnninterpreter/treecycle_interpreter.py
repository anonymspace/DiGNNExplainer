...
NUM_CLASSES = 2
MAX_NODES = 15
DATASET_NAME = 'TreeCycle'
CLASSES = [0, 1]

def test_interpreter(target_class, show=False):
    dataset = TreeCycledataset(seed=12345)
    ...
    for seed in [15]:
        ...
        model.load_state_dict(torch.load('ckpts/treecycle_ckpt.pt'))
...
def get_trainer(dataset,model,mean_embeds,seed):
    ...
    trainer[cls_idx] = TrainerNC(
        ...
        scheduler=torch.optim.lr_scheduler.ExponentialLR(o, gamma=1),
        dataset=dataset,
        budget_penalty=BudgetPenalty(budget=500, order=2, beta=1),
        target_probs={cls_idx: (0.9, 1)},
        k_samples=10
    )
...
if __name__ == "__main__":
    ...
    print('Average faithfulness for 10-15 node sizes', np.mean(mean_faithfulness_list))
