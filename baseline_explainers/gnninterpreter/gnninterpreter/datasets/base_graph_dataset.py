...
class BaseGraphDataset(pyg.data.InMemoryDataset, ABC):
    ...
    def convert_nc(self, G, generate_label=False):
        if isinstance(G, list):
            return pyg.data.Batch.from_data_list([self.convert(g) for g in G])
        G = nx.convert_node_labels_to_integers(G)
        node_labels = [G.nodes[i]['label']
    ...
    def fit_model_nc(self, model, batch_size=1, lr=0.01):
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss()
        model.train()
        losses = []
        for batch in self.loader(batch_size=batch_size, shuffle=True):
            ...
            train_idx = batch.y != -1
            out= out['logits']
            loss = criterion(out[train_idx], batch.y[train_idx])  # Compute the loss.
            loss.backward()  # Derive gradients.
            optimizer.step()  # Update parameters based on gradients.
            losses.append(loss.item())
        return np.mean(losses)
    #Added for ba3
    @torch.no_grad()
    def test(self,model,testsize,batch_size=32):
        model.eval()
        ...
        return total_correct / testsize
    #Added for nc datasets
    @torch.no_grad()
    def test_nc(self,model,batch_size=32):
        model.eval()
        ...
        for batch in self.loader(batch_size=batch_size, shuffle=False):
            ...
            accs = []
            for mask in [batch.train_mask, batch.val_mask, batch.test_mask]:
                accs.append(int((pred[mask] == batch.y[mask]).sum()) / int(mask.sum()))
            return accs
    ...
    def mean_embeddings_nc(self, dataset, model, batch_size=32):
        ...
        #for bashapes, tree-cycle, tree-grids use below code
            ...
            embeds = [[] for _ in range(len(self.NODE_CLS))]
        model.eval()

        for batch in self.loader(batch_size=batch_size, shuffle=False):
            # print(model(batch)['embeds'])
            for i, e in enumerate(model(batch)['embeds']):
                embeds[i].append(e)
        return [torch.stack(e, dim=0).mean(axis=0) for e in embeds]
