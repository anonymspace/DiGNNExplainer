...
class TrainerNC:
    ...
    @torch.no_grad()
    def predict(self, G):
        batch = pyg.data.Batch.from_data_list([self.dataset.convert_nc(G, generate_label=True)])
    ...
    def evaluateAndLog(self, *args, ax=None, log_file_probs=None, **kwargs):
        ...
        stats = dict(label=list(self.dataset.NODE_CLS.values()),
                    mean=np.mean(p, axis=0),
                    std=np.std(p, axis=0))
    ...
    def show(self, G, ax=None):
        ...
        #added to get the class for avg prob
        cls = list(self.target_probs.keys())[0]
        settings.class_prob_list.append(probs[cls])
        print(f"{n=} {m=}")
        print(f"{logits=}")
        print(f"{probs=}")
        self.dataset.draw(G, ax=ax)
