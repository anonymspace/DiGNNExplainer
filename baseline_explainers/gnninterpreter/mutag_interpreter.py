...
def test_interpreter(target_class, show=False):
    ...
    for seed in [15]:
        ...
        model.load_state_dict(torch.load('ckpts/mutag_ckpt.pt'))
...
def get_trainer(dataset,model,mean_embeds,seed):
    ...
    trainer[cls_idx] = Trainer(
        ...
        criterion=WeightedCriterion([
            ...
            dict(key="logits", criterion=MeanPenalty(), weight=0),
            dict(key="omega", criterion=NormPenalty(order=1), weight=5),   #L1
            dict(key="omega", criterion=NormPenalty(order=2), weight=2),    #L2
            dict(key="xi", criterion=NormPenalty(order=1), weight=5),  # L1
            dict(key="xi", criterion=NormPenalty(order=2), weight=2),   #L2
            dict(key="theta_pairs", criterion=KLDivergencePenalty(binary=True), weight=2),  #Rc,
        ]),
        optimizer=(o := torch.optim.SGD(s.parameters(), lr=1)),
        scheduler=torch.optim.lr_scheduler.ExponentialLR(o, gamma=1),
        dataset=dataset,
        budget_penalty=BudgetPenalty(budget=10, order=2, beta=1),           #Rb
        target_probs={cls_idx: (0.9, 1)},
        k_samples=10                                                        #K
    )
...
def get_faithfulness(graph_list):
    ...
    for i, expln_graph in enumerate(graph_list):
        ...
        if i == 0:
            motifs_path = '../../evaluation/motifs/mutag/class0'
        elif i == 1:
            motifs_path = '../../evaluation/motifs/mutag/class1'
...
if __name__ == "__main__":
    ...
    print('Average faithfulness for 6 node size', np.mean(faithfulness_list))
    print('Average probability for 6 node size', np.mean(avg_prob_list))
