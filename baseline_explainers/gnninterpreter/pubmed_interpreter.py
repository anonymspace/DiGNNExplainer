...
def test_interpreter(target_class, show=False):
    dataset = PubMeddataset(seed=12345)
    ...
    for seed in [15]:
        ...
        model.load_state_dict(torch.load('ckpts/pubmed_ckpt.pt'))
        ...
        if target_class == 0:
            ...
            #     for i in range(1):
            #         trainer[3].evaluate(threshold=0.5, show=True, bernoulli=True, connected=True)
            expln_graph = trainer[4].evaluate(threshold=0.5, show=True, bernoulli=True, connected=True)
            print("=" * 30)
            ...
            #     for i in range(1):
            #         trainer[3].evaluate(threshold=0.5, show=True, bernoulli=True, connected=True)
            expln_graph = trainer[5].evaluate(threshold=0.5, show=True, bernoulli=True, connected=True)
            print("=" * 30)
            ...
            #     for i in range(1):
            #         trainer[3].evaluate(threshold=0.5, show=True, bernoulli=True, connected=True)
            expln_graph = trainer[6].evaluate(threshold=0.5, show=True, bernoulli=True, connected=True)
            print("=" * 30)
            ...
            #     for i in range(1):
            #         trainer[3].evaluate(threshold=0.5, show=True, bernoulli=True, connected=True)
            expln_graph = trainer[7].evaluate(threshold=0.5, show=True, bernoulli=True, connected=True)
            print("=" * 30)
...
def get_trainer(dataset,model,mean_embeds,seed):
    ...
    trainer[cls_idx] = TrainerNC(
        ...
        criterion=WeightedCriterion([
            ...
            dict(key="embeds", criterion=EmbeddingCriterion(target_embedding=mean_embeds[cls_idx]), weight=1),
            # 1 similarity to original dataset
            dict(key="logits", criterion=MeanPenalty(), weight=0),
            dict(key="omega", criterion=NormPenalty(order=1), weight=10),
            dict(key="omega", criterion=NormPenalty(order=2), weight=5),
