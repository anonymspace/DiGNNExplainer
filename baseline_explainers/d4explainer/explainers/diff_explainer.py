...
class DiffExplainer(Explainer):
    ...
    def one_step_model_level(self, args, adj, node_feature, target_class):
        ...
        for i, sigma in enumerate(sigma_list):
            ...
            if args.task == "nc":
                ...
                output_prob, _ = self.model.get_node_pred_subgraph(
                    ...
                    mapping=torch.ones(args.nodes, dtype=torch.bool), #6
                )
                y_pred = output_prob.argmax(dim=-1)
                correct = (y_pred == torch.full((1,args.nodes), target_class).to(args.device)) #6
    ...
    def multi_step_model_level(self, args, target_class, node_feature_size):
        n = args.nodes   # nodes
        m = n / 2  # edges
        ...
        for i in range(steps, 1, -1):
            ...
            softmax, pred_adj = self.one_step_model_level(args,adj, node_feature, target_class)
            adj = pred_adj
