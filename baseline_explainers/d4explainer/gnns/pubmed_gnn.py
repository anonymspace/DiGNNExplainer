...
def parse_args():
    ...
    parser.add_argument("--data_name", nargs="?", default="pubmed", help="Input data path.")
    parser.add_argument(
        "--model_path",
        nargs="?",
...
class PubMed_GCN(torch.nn.Module):
    def __init__(self, num_unit, n_input, n_out, n_hid):
        super(PubMed_GCN, self).__init__()
...
if __name__ == "__main__":
    ...
    model = PubMed_GCN(args.num_unit, n_input=n_input, n_out=n_labels, n_hid=args.hidden).to(device)
