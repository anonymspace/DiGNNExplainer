...
class Mutag_GCN(torch.nn.Module):
    def __init__(self, conv_unit=3):
        ...
        self.convs.append(GCNConv(in_channels=7, out_channels=32))
        for i in range(conv_unit - 2):
            self.convs.append(GCNConv(in_channels=32, out_channels=32))
        self.convs.append(GCNConv(in_channels=32, out_channels=32))

        self.batch_norms.extend([BatchNorm(32)] * conv_unit)
        self.relus.extend([ReLU()] * conv_unit)

        # self.lin1 = Lin(128, 128)
        self.ffn = nn.Sequential(*([nn.Linear(32, 32)] + [ReLU(), nn.Dropout(), nn.Linear(32, 2)]))
